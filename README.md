...
**Domain:** `Medical / Medication Safety` · **Suite:** `Medical & Research Suite` · **Accent:** `#5eead4`
...
**Drug Interaction Api** is a standalone, browser-based AI workflow app for **Medical / Medication Safety**. It turns structured inputs, uploaded files, and project-specific settings into reviewable outputs using a deterministic local engine plus optional local/cloud LLM enhancement.
...
|---|---:|:---:|---|---|
| `medication_list`<br>Medication list | text | Yes | — | Affects input: Medication list. |
...
| `country_database`<br>country database | select | No | United States | Affects customization: country database. |
| `age_group`<br>age group | text | No | — | Affects customization: age group. |
| `pregnancy_renal_hepatic_context`<br>pregnancy/renal/hepatic context | text | No | — | Affects customization: pregnancy/renal/hepatic context. |
| `severity_threshold`<br>severity threshold | slider | No | 50 | Affects customization: severity threshold. |
| `output_audience`<br>output audience | select | No | general | Affects customization: output audience. |
...
- **country database**: United States, United Kingdom, Egypt, UAE, Saudi Arabia, Germany, Custom
- **output audience**: general, executive, technical, legal, medical, student, customer
...
uvicorn app.main:app --host 127.0.0.1 --port 9118
```
...
| `AI_SUITE_PORT` | Port for this project GUI, default `9118`. |
...
- Export download
- Job history
- RxNorm/RxNav/OpenFDA/full interaction DB required for clinical completeness
...
```bash
curl http://127.0.0.1:9118/api/health
```
...
```bash
curl -X POST http://127.0.0.1:9118/api/run \
...
```bash
AI_SUITE_HOST=0.0.0.0 AI_SUITE_PORT=9118 uvicorn app.main:app --host 0.0.0.0 --port 9118
```
...
- `plugins/drug-interaction-api.json` for plugin metadata.
